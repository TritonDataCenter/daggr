...
function FilterStream(args, filter)
{
	var fs = this;
	mod_events.EventEmitter.call(this);

	this.fs_json = args['json'];
	...
	this.fs_source.on('end', this.emit.bind(this, 'end'));
}
...
function Printer(args)
{
	...
	var outputs = this.p_outputs = args['outputs'].slice(0);
	var json = this.p_json = args['mode'] == 'json';
